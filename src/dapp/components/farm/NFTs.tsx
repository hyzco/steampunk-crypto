...
export const NFTs: React.FC<Props> = ({ inventory }) => {
  const now = new Date();
  const hasInventoryChristmasTree = inventory && inventory["Christmas Tree"] > 0;
  ...
  return (
    <>
      <div id="statue">
        {inventory && inventory["Sunflower Statue"] > 0 && (
          <img src={statue} alt="statue" />
        )}
      </div>

      <div id="potato-statue">
        {inventory && inventory["OG Potato Statue"] > 0 && (
          <img src={potatoStatue} alt="statue" />
        )}
      </div>

      <div id="scarecrow">
        {inventory && inventory["Scarecrow"] > 0 && (
          <img src={scarecrow} alt="statue" />
        )}
      </div>

      <div id="farm-cat">
        {inventory && inventory["Farm Cat"] > 0 && <img src={farmCat} alt="farmCat" />}
      </div>

      <div id="farm-dog">
        {inventory && inventory["Farm Dog"] > 0 && <img src={dog} alt="dog" />}
      </div>

      <div id="farm-gnome">
        {inventory && inventory["Gnome"] > 0 && <img src={gnome} alt="gnome" />}
      </div>
